 ...
 */
/**
 * `?diag=1` — what is painting near the top of the screen, on *your* device.
 *
 * Puzz reported a 1px line across the top of the game on an iPhone. Measured
 * off his screenshot it is rows 49-51 of 1036 running the full width, three
 * device pixels at DPR 3, in `rgb(27,52,82)` — which is this stylesheet's own
 * `--line` under JPEG noise, so it is ours rather than something iOS draws.
 *
 * It does not reproduce. Rendering production at his exact viewport — 932x345
 * CSS at DPR 3, in cockpit — and enumerating every element *and pseudo-element*
 * that paints a border, background or shadow within the top 40px finds nothing
 * wider than a 340px panel, and `elementFromPoint(700, 16)` is the canvas.
 *
 * The one variable a headless Chrome cannot reproduce is `env(safe-area-inset-*)`,
 * which is 0 here and is not 0 on a notched phone held sideways. So rather than
 * guess again, this asks the device: it prints the insets it actually has, and
 * every box that paints anything across more than half the width up there, in a
 * panel that fits in a screenshot.
 *
 * Deliberately behind a query parameter and not a build flag — the person who
 * can see the bug is not the person who can rebuild.
 */
function runDiagnostics(): void {
  ...
  const read = () => {
    ...
    const insets = [cs.paddingTop, cs.paddingRight, cs.paddingBottom, cs.paddingLeft].join(' ')
    probe.remove()

    const w = window.innerWidth
    const lines: string[] = [
      `viewport ${w}x${window.innerHeight} dpr ${devicePixelRatio}`,
      `insets(t r b l) ${insets}`,
      `standalone ${(navigator as unknown as { standalone?: boolean }).standalone ?? '?'}`,
      `compact ${document.body.classList.contains('hud-compact')}`,
      ...
      `visual ${Math.round(visualViewport?.width ?? 0)}x${Math.round(visualViewport?.height ?? 0)}` +
        ` off ${Math.round(visualViewport?.offsetTop ?? 0)}`,
      // `elementsFromPoint`, plural, minus this panel — otherwise the panel is
      // the answer to every question, which is what the first run reported.
      ...[4, 8, 12, 16, 20, 24].map(
        (y) =>
          `at(y=${y}) ` +
          document
            .elementsFromPoint(Math.round(w / 2), y)
            .filter((el) => el.id !== 'diag')
            .slice(0, 3)
            .map(describe)
            .join(' < '),
      ),
      '--- paints in top 40px, wider than half the screen ---',
    ]

    const seen = (el: Element, pseudo: string | null) => {
      const cs2 = getComputedStyle(el, pseudo)
      if (cs2.display === 'none' || cs2.visibility === 'hidden') return
      if (pseudo && (cs2.content === 'none' || cs2.content === 'normal')) return
      const paints =
        cs2.borderTopWidth !== '0px' ||
        cs2.borderBottomWidth !== '0px' ||
        cs2.backgroundColor !== 'rgba(0, 0, 0, 0)' ||
        cs2.boxShadow !== 'none' ||
        (cs2.backgroundImage !== 'none' && cs2.backgroundImage !== '')
      if (!paints) return
      const r = el.getBoundingClientRect()
      const wide = pseudo ? parseFloat(cs2.width) > w / 2 : r.width > w / 2
      const near = pseudo ? true : r.top <= 40 && r.bottom >= -2
      if (!wide || !near) return
      lines.push(
        `${describe(el)}${pseudo ?? ''} box[${Math.round(r.left)},${Math.round(r.top)},` +
          `${Math.round(r.width)}x${Math.round(r.height)}] ` +
          `bt:${cs2.borderTopWidth} bb:${cs2.borderBottomWidth} ` +
          `c:${cs2.borderTopColor} bg:${cs2.backgroundColor}`,
      )
    }
    for (const el of document.querySelectorAll('html, body, #stage, #hud, #hud *, #lobby, #board')) {
      seen(el, null)
      seen(el, '::before')
      seen(el, '::after')
    }
  ...
  // Re-read on a timer rather than on a tap. The panel is `pointer-events:
  // none` because it sits over the lobby's play button, so there is no tap to
  // listen for — and most of the HUD does not exist until a game is running.
...
if (params.get('diag') === '1') runDiagnostics()
