...
- **Modes** — deathmatch and team deathmatch, picked on the lobby as cards.
...
- **The leaderboard** — a chain of blocks rather than a table, with seasons derived from
  difficulty epochs.
- **Phones** — the board is the screen, panels collapse to chips, on-screen sticks, and
...
1. **Kill streaks you trigger rather than receive** — earn a reward into a tray of
   ...
   happens *to* you, and spending it is the entire decision.
2. **The phone HUD** — the feed draws straight through the ammo pips and the rules
   banner is squeezed to a 30px sliver, so on a phone you cannot read what mode you are
   playing. Two layout faults, both visible in one screenshot.
3. **Team deathmatch does not really work** — a mode that is on the hub and does not
   deliver is worse than one that is not there.
4. **The air strike does not visibly land in a real match** — the marquee reward at
   five kills, and it needs to be seen to be worth earning.
5. **Kill streaks are invisible to everybody but the earner** — a chopper nobody else
   can see is a private cutscene, not a threat.
6. **Custom skins and a picker that holds dozens** — upload your own art, save a
   collection, and a picker that does not fall over at thirty entries.
7. **Per-mode leaderboards** — a flag capture and a solo kill are not the same unit,
   and with CTF coming there will be three games measured on one axis.
8. **Capture the flag** — the third mode, behind the selector that already advertises
   it.
...
**Feel** — a kill cam from the killer's view, a kill feed with a skull and both faces,
more power-ups, a longer chopper, carpet bombing retimed so it does not start on a
corpse, bot difficulty levels.
...
- `test/arsenal.mjs` — still expects overdrive at ten. It has been the chopper since
  `940f906`.
